...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .scenario,.card,.verdict {{ margin:22px 0 0; padding:22px 0; border-top:1px solid var(--ink); background:#fff; }} .scenario strong {{ display:block; font-size:28px; max-width:34ch; }} .confidence,.evidence {{ font-weight:800; }} .guardrail,.caveat {{ background:#fff8dc; padding:15px; }} .card img {{ display:block; max-width:100%; height:auto; }} .check-list {{ list-style:none; margin:22px 0; padding:0; border-top:1px solid var(--line); }} .check-list li {{ display:grid; grid-template-columns:22px minmax(0,1fr); gap:10px; padding:12px 0; border-bottom:1px solid var(--line); }} .check-list li::before {{ content:"✓"; color:var(--green); font-weight:800; }}
...
def write_fast_lineage_report(
    ...
    recombination_class = "stop" if detected else "review"
    if detected:
...
.status{{display:inline-block;max-width:100%;border:2px solid var(--red);color:var(--red);padding:9px 13px;font-weight:800}}section{{display:grid;grid-template-columns:180px minmax(0,1fr);border-bottom:2px solid var(--ink)}}
...
.stage-head{{display:flex;justify-content:space-between;align-items:start;gap:24px}}.decision{{border:2px solid currentColor;padding:9px 12px;font-size:12px;font-weight:800;letter-spacing:.06em;text-align:center;max-width:220px}}.decision.stop{{color:var(--red)}}.decision.review{{color:var(--blue)}}

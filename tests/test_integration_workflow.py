...
def test_small_workflow_reaches_time_scaled_and_location_trees(tmp_path: Path) -> None:
    ...
    assert (output / "E_coli__ST_test" / "recombination_intervals.tsv").is_file()
    assert (output / "E_coli__ST_test" / "recombination_genome_profile.csv").is_file()
    assert (output / "E_coli__ST_test" / "recombination_summary.json").is_file()
    assert (output / "E_coli__ST_test" / "recombination_map.svg").is_file()
    assert (output / "E_coli__ST_test" / "recombination_map.png").is_file()
